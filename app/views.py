from flask import render_template,flash,redirect,session,url_for,request,abort,g,jsonify
...
def getdata():
	...
	if source=="Github":	
			try:
				...
				print data
				for x in data:
					s=s+"<li>"+x["name"]+"<br><a href="+x["html_url"]+">"+x["html_url"]+"</a></li>"
			#		s=s+"<li>"+x['name']+"</li>"
			except:
				return jsonify({1:"error to load items"})
	elif source=="Twitter":
			try:
                        	datas=tweets(username,40)
				s=''
                                for x in datas:
                                        s=s+'<li>'+x['text']+'</li>'
                        except:
                                return jsonify({1:"error to load items"})
    	s='<h2>'+source+'</h2>'+'<ol>'+s+'</ol>'
	print s
    	return jsonify({1:s})
...
def index():
	...
	if request.form.get('submit') and source=='Github':
                try:
                        datas=json.loads(urlopen("https://api.github.com/users/"+request.form.get('input')+"/repos").read())
                except:
                        None
	elif request.form.get('submit') and source=='Twitter':
		print request.form.get('select')
		try:
			datas=tweets(request.form.get('input'),40)
		except:
			None
	return render_template("index.html",title='Home',datas=datas,source=source)
